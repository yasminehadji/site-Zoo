...
if ($fonction === "Responsable Boutique") {
    $sql = "SELECT cj.id_boutique,
                   ...
                   EXTRACT(MONTH FROM cj.date_ca) AS mois,
                   SUM(cj.chiffre_affaire) AS ca_mensuel
    ...
    $sql = "SELECT cj.id_boutique,
            ...
            GROUP BY cj.id_boutique, b.nom_boutique,
                     EXTRACT(YEAR FROM cj.date_ca),
                     EXTRACT(MONTH FROM cj.date_ca)
            ORDER BY cj.id_boutique, annee DESC, mois DESC";
    $stmt = oci_parse($conn, $sql);
}
        ...
        <table>
            ...
            <?php while ($row = oci_fetch_assoc($stmt)): ?>
                <tr>
                    ...
                    <td><?php echo htmlspecialchars($row['CA_MENSUEL']); ?> €</td>
                </tr>
            <?php endwhile; ?>
        </table>
    </div>
